$(document).ready(function(){
    ...
    /*********header와 메뉴 시작 *****************
        pc 인지 모바일인지 구분 브라우저 넓이로
        스크롤값 계산
        공통사항 : 브라우저가 스크롤 되면 OR header에 오버하면 header에 fixed 클래스를 추가
        PC일때 : 마우스를 오버한 li에만 over 클래스 추가
        모바일때 : 메뉴열기를 클릭하면 header에 menu_open 클래스 추가
                  1차메뉴를 클릭하면 (하위메뉴가 있는 1차메뉴) 클릭한 li에 open 클래스 추가

    *************header와 메뉴 종료*********/

    let device_status // 모바일 인지 pc 인지
    ...
    resize_chk() //함수실행
    $(window).resize(function(){ //브라우저가 리사이즈 될떄마다 1번씩 실행
        resize_chk() //함수실행 
    })
    ...
    */
    ...
    $('.visual .btn_wrap button.btn_stop').on('click', function(){
        ...
        $('.visual .btn_wrap button.btn_play').show() //재생 버튼 보이게
    })
    $('.visual .btn_wrap button.btn_play').on('click', function(){
        //console.log('재생 버튼 클릭')
        visual_swiper.autoplay.start(); /* 재생 기능 */
        $(this).hide() //정지버튼 자신은 숨김
        $('.visual .btn_wrap button.btn_stop').show() //재생 버튼 보이게
    })
    // visual swiper 종료
    ...
    // 4. 클릭한 li속성aria-selected값을 true로 바꿔줘야함 (다른 li는 모두 false)
    let find_content //클릭한 메뉴의 이름
    ...
    const adopt_swiper = new Swiper('.adopt .swiper', { /* 팝업을 감싼는 요소의 class명 */
        ...
        breakpoints: {
            768: {    /* 640px 이상일때 적용 */
                // slidesPerView: 7,    /*    'auto'   라고 쓰면 css에서 적용한 넓이값이 적용됨 */
                spaceBetween: 24,
            },
        },
        ...
        navigation: {
            ...
            prevEl: '.adpot .list_ctrl .btn_prev',
        },
    });
    /****************  분양 swiper 종료**********************/
    ...
    $('footer .top span').on('click', function(){
        console.log('클릭')
        $('html, body').animate({ /* 이동할 오브젝트 */
            scrollTop: 0 /* 스크롤 값 */
        }, 500) /* 애니메이션 시간 */
    })
