$(document).ready(function(){
    ...
    /*
        ...
        3. 다른메뉴에 마우스를 오버하면 이전에 오버했던 메뉴는 아웃됨
           (over 클래스 삭제)
           removeClass
        4. 메뉴 영역을 벗어나면 herder/li에 들어간 클래스 삭제
    */
   //$('header .gnb .gnb_wrap ul.depth1 > li').addClass('over'), function(){}
   //$('header .gnb .gnb_wrap ul.depth1 > li').removeClass('over'), function(){}
    $('header .gnb .gnb_wrap ul.depth1 > li').on('mouseenter focusin', function(){
        if (device_status == 'pc') { //pc일때만
            $('header').addClass('menu_over')

            /* 이전에 마우스를 오버했던 li에서는 over를 삭제해야 하는데
            이전에 마우스르 오버했던 li를 찾는 것이 다소 복잡
            일단 모든 li에 있는 over 클래스를 모두 지우고
            마우스를 오버한 li에만 over를 추가함...
            이럼 이전에 오버했던 li를 정확하게 몰라도 됩니다. 
    ...
    */
    ...
    /*
        메뉴 열기 버튼을 클릭하면 메뉴가 열림
        -- header .gnb .gnb_open 를 클릭하면 header에 menu_open 클래스 추가
        메뉴 닫기 버튼을 클릭하면 메뉴가 닫힘
        -- header .gnb .gnb_close 클릭하면 header에 menu_open클래스 삭제
    */
